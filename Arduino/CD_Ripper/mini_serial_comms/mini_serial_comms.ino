...
const uint8_t limt_angles[5][2] = {{0,95},{0,180},{0,180},{25,180},{0,180}}; /* 各个关节角度的限制(angle limit for each angle) */
...
void user_task(void)
{
  ...
  static uint32_t gr = 1;
  static uint32_t _awaiting = 0;
  ...
  if (Serial.available()) {
    ...
    if (cmd == "OP:LAUNCH") {
      gc = 1;
      Serial.println("WW:ROGER");
    }
    else if (cmd == "OP:PANG") {
      gc = 3;
      Serial.println("WW:LING");
    }
    else if (cmd == "PING") {
      Serial.println("WW:PONG");
    }
    _awaiting = 0;
  ...
  switch(step)
  {
    case 0:
      ...
      if (gc == 0){
        Serial.print("WW:action run.");
        Serial.println(gc);
        break;
      }

      action_ctl.action_set(gc);//执行动作组1(execute action group 1)
      Serial.print("WW:action run.");
      step = 1;
      ////gc = (gc%2) + 1;
      Serial.println(gc);
    ...
    case 1:
      ...
      {
        //Serial.println("");
        Serial.print("OK:The action group ");
        Serial.print(gc);
        Serial.println(" ran successfully!");
        step = 0;
        gc = 0;
      }else{
        Serial.print("WW: .");
...
void servo_control(void) {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    {
      servo_angles[i] = servo_angles[i] * 0.95 + servo_expect[i] * 0.05;
      if(servo_angles[i] < servo_expect[i])
        servo_angles[i] = servo_expect[i];
    }else if(servo_angles[i] < servo_expect[i])
    {
      servo_angles[i] = servo_angles[i] * 0.95 + (servo_expect[i] * 0.05 + 1);
      if(servo_angles[i] > servo_expect[i])
        servo_angles[i] = servo_expect[i];
    }
